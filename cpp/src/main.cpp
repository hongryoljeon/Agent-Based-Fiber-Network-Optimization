...
int main() {
    // --- 1. Parameters (Identical to Original Python) ---
    ...
    const double thres = 1e-8; // Global segment threshold
    const double thr = 0.01;   // LS rule
    ...
    for (int en = 0; en < ensemble_size; ++en) {
        ...
        // Initial 4 edges [start_b, start_m, end_b, end_m, k, L0]
        for(int i=0; i<4; ++i) {
            VectorXd e_init(6);
        ...
        while (y < num_simulations) {
            std::cout << "Ensemble: " << en << " | Step: " << y << std::endl;

            while (true) { // Structural Validity Search (Rollback Loop)
                ...
                // --- 2. Agent Decisions & Vision Rule (LS Rule) ---
                for (int n = 0; n < num_agents; ++n) {
                    ...
                    while (!agent_moved) {
                        ...
                        MatrixXd b_tmp(nodes_b.size(), 2); for(int k=0; k<nodes_b.size(); ++k) b_tmp.row(k)=nodes_b[k];
                        MatrixXd m_tmp(nodes_m.size(), 2); for(int k=0; k<nodes_m.size(); ++k) m_tmp.row(k)=nodes_m[k];
                        ...
                        // LS Rule (thr) check using vision logic
                        ...
                        // t identifier tracking (Crucial for splitting)
                ...
                // --- 3. MAPO, MAPX, DELETE, SPLIT LOGIC (The Heart of Simulation) ---
                if (!node_intersection_points.empty()) {
                    std::vector<int> mapo(num_agents, 1), mapx(num_agents, 0);
                    ...
                    for (int i = 0; i < (int)edges.size(); ++i) {
                        ...
                        for(auto& nip : node_intersection_points) {
                            // Python: i < len(e)-num_agents else arr[(arr[:,3]==i)|(arr[:,2]==i)]
                            if (i < (int)edges.size() - num_agents) {
                        ...
                        // Mapo / Mapx Tracking for Agent Index
                        for(int n=0; n<num_agents; ++n) {
                    ...
                    // Perform Deletion (Reverse order to maintain indices)
                    ...
                    // Final THRES check
                    MatrixXd b_f(nodes_b.size(), 2); for(int k=0; k<nodes_b.size(); ++k) b_f.row(k)=nodes_b[k];
                    MatrixXd m_f(nodes_m.size(), 2); for(int k=0; k<nodes_m.size(); ++k) m_f.row(k)=nodes_m[k];
            ...
            // --- 4. Optimization (L-BFGS-B Implementation) ---
            if (!nodes_m.empty()) {
                MatrixXd b_opt(nodes_b.size(), 2); for(int i=0; i<nodes_b.size(); ++i) b_opt.row(i) = nodes_b[i];
                MatrixXd e_opt(edges.size(), 6); for(int i=0; i<edges.size(); ++i) e_opt.row(i) = edges[i];
                VectorXd x(nodes_m.size() * 2); for(int i=0; i<nodes_m.size(); ++i) x.segment<2>(i*2) = nodes_m[i];

                LBFGSParam<double> param; param.epsilon = GTOL;
                LBFGSSolver<double> solver(param);
                FiberObjective obj(b_opt, e_opt, C_val);
                double final_e;
                int niter = solver.minimize(obj, x, final_e);

                for(int i=0; i<nodes_m.size(); ++i) nodes_m[i] = x.segment<2>(i*2);
                
                VectorXd final_grad; obj(x, final_grad);
                Utils::check_convergence(true, "Optimized", final_e, final_grad, niter, GTOL);
            }

            // --- 5. Agent Position Update (mapo/mapx index logic) ---
            for(int n=0; n<num_agents; ++n) {
                if (!node_intersection_points.empty() && new_lin_nums[n] > 3) {
                    std::vector<int> indices;
                    for(int idx = (int)edges.size()-mapx[n]-1; idx >= (int)edges.size()-mapo[n]-mapx[n]; --idx) {
                        indices.push_back(idx);
                    }
                    std::vector<double> probs;
                    MatrixXd b_m(nodes_b.size(), 2); for(int k=0; k<nodes_b.size(); ++k) b_m.row(k)=nodes_b[k];
                    MatrixXd m_m(nodes_m.size(), 2); for(int k=0; k<nodes_m.size(); ++k) m_m.row(k)=nodes_m[k];
            ...
            // --- 6. Save Data ---
            if (y % 5 == 0) {
                MatrixXd b_s(nodes_b.size(), 2); for(int k=0; k<nodes_b.size(); ++k) b_s.row(k)=nodes_b[k];
                MatrixXd m_s(nodes_m.size(), 2); for(int k=0; k<nodes_m.size(); ++k) m_s.row(k)=nodes_m[k];
                MatrixXd e_s(edges.size(), 6); for(int i=0; i<edges.size(); ++i) e_s.row(i)=edges[i];
